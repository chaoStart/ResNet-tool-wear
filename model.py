...
def build_multi_input_main_residual_network(
    ...
    print('a2_inp、d2_inp、d1_inp的形状和类型:',type(a2_inp),type(d2_inp),type(d1_inp))
    print('a2_inp、d2_inp、d1_inp的形状和类型:',a2_inp.shape,d2_inp.shape,d1_inp.shape)
    print('out输出是什么:',type(out),out.shape)
...
def build_2d_main_residual_network(batch_size,
    ...
    model = Model(inp,out)
    # model.compile(loss='mse',optimizer='adam',metrics=['mse','mae'])
    model.compile(loss='mse',optimizer='adam_v2.Adam',metrics=['mse','mae'])
    return model
